...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	mp3DecInfo_pointer = (MP3DecInfo *)os_alloc_mem(AppMalloc, sizeof(MP3DecInfo));
	if (!mp3DecInfo_pointer)
		return 0;
	ClearBuffer(mp3DecInfo_pointer, sizeof(MP3DecInfo));
	
	fh =  (FrameHeader *)     os_alloc_mem(AppMalloc, sizeof(FrameHeader));
	si =  (SideInfo *)        os_alloc_mem(AppMalloc, sizeof(SideInfo));
	sfi = (ScaleFactorInfo *) os_alloc_mem(AppMalloc, sizeof(ScaleFactorInfo));
	hi =  (HuffmanInfo *)     os_alloc_mem(AppMalloc, sizeof(HuffmanInfo));
	di =  (DequantInfo *)     os_alloc_mem(AppMalloc, sizeof(DequantInfo));
	mi =  (IMDCTInfo *)       os_alloc_mem(AppMalloc, sizeof(IMDCTInfo));
	sbi = (SubbandInfo *)     os_alloc_mem(AppMalloc, sizeof(SubbandInfo));
...
#define SAFE_FREE(x)	{if (x)	os_free_mem(AppMalloc, x);	(x) = 0;}	/* helper macro */
#endif
