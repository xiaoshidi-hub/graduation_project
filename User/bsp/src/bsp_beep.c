...
//#define BEEP_HAVE_POWER		/* 定义此行表示有源蜂鸣器，直接通过GPIO驱动, 无需PWM ，因为V6板子上面是无源蜂鸣器，所以把这一行注释掉*/
...
void BEEP_Start(uint16_t _usBeepTime, uint16_t _usStopTime, uint16_t _usCycle)
{
	...
	g_tBeep.usBeepTime = _usBeepTime;//鸣叫时间，单位 10ms
	g_tBeep.usStopTime = _usStopTime;//停止鸣叫时间，单位 10ms
	g_tBeep.usCycle = _usCycle;      //鸣叫和停止的循环次数
	g_tBeep.usCount = 0;             //用于鸣叫和停止时的计数。
	g_tBeep.usCycleCount = 0;        //用于循环次数计数
	g_tBeep.ucState = 0;             //状态变量，用于蜂鸣器鸣叫和停止的区分
	g_tBeep.ucEnalbe = 1;	/* 设置完全局参数后再使能发声标志 *///用于使能或者禁止蜂鸣器
