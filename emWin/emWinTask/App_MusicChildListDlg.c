...
*	                                  
*	模块名称 : 音乐播放器歌曲列表界面
*	文件名称 : App_MusicChildListDlg.c
*	版    本 : V1.0
*	说    明 : 这个对话框是音乐对话框的子窗口，用于显示歌曲列表
*              1. 支持上一曲，下一曲，快进和快退，常用的采样率和码率都支持，单声道和立体声也都支持。
*              2. emWin任务是低优先级任务，音乐解码任务是高优先级任务，两个任务之间通过消息邮箱和事件
*                 标志进行通信。
...
*********************************************************************************************************
*/
WM_HWIN hWinMusicList = WM_HWIN_NULL;             /* 歌曲列表对话框句柄 */
extern const char s_MusicPathDir[];               /* 歌曲在存储器中的存放路径 */
...
{
    { FRAMEWIN_CreateIndirect,    "歌曲列表",        0,             120,   10,  559,  329,  0},
...
static void InitDialogMusicList(WM_MESSAGE * pMsg)
{
    ...
    LISTVIEW_AddColumn(hItem, 60, "歌曲名（文件名仅支持ASCII字符）", GUI_TA_VCENTER|GUI_TA_LEFT);//添加一列数据
    LISTVIEW_AddColumn(hItem, 60, "播放时间", GUI_TA_VCENTER|GUI_TA_LEFT);//添加一列数据
    LISTVIEW_AddColumn(hItem, 60, "文件大小", GUI_TA_VCENTER|GUI_TA_LEFT);//添加一列数据
    LISTVIEW_SetColumnWidth(hItem, 0, 330);//设置列宽
    LISTVIEW_SetColumnWidth(hItem, 1, 105);//设置列宽
    LISTVIEW_SetColumnWidth(hItem, 2, 105);//设置列宽
