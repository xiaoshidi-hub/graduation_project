...
static void _cbDialogCalculator(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
	...
	FRAMEWIN_SetTextAlign(hItem, GUI_TA_HCENTER | GUI_TA_TOP);//设置标题在中间
	FRAMEWIN_SetTitleHeight(hItem, 40);                       //设置标题框的高度
	FRAMEWIN_SetFont(hItem, &GUI_FontHZ16);                  //设置标题框的字体格式
	FRAMEWIN_SetText(hItem, "计算器");	  
	FRAMEWIN_AddCloseButton(hItem,FRAMEWIN_BUTTON_RIGHT,0);   //在标题栏添加一个关闭按钮
//	FRAMEWIN_AddMinButton(hItem,FRAMEWIN_BUTTON_RIGHT,2);     //在标题栏添加一个最小化按钮
