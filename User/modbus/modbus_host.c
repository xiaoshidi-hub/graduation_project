...
{	
	...
	if(xResult == OS_R_TMO)
	{
		printf_modbusdbg("MODH_Poll->溢出时间\r\n");
		return 0;			
	}
	...
	*/
	...
	if (crc1 != 0)
	{
		printf_modbusdbg("MODH_Poll->校验失败 -- %d\r\n",  g_tModH.RxCount);
...
err_ret:
	...
	printf_modbusdbg("MODH_Poll->退出g_tPrint.Rxlen = %d  ret = %d\r\n", g_tPrint.Rxlen, ret);
	return ret;
}
...
uint8_t MODH_ReadParam_01H(uint16_t _reg, uint16_t _num)
{
	...
	for (i = 0; i < NUM; i++)
	{
		...
		while (1)				/* 等待应答,超时或接收到应答则break  */
		{
			...
			{
				printf_modbusdbg("接收到应答g_tModH.fAck01H = %d\r\n", g_tModH.fAck01H);
				break;		/* 接收到应答, 能够进入到这里，说明校验已经成功了 */
			}
		}
...
uint8_t MODH_ReadParam_02H(uint16_t _reg, uint16_t _num)
{
	...
	for (i = 0; i < NUM; i++)
	{
		MODH_Send02H (SlaveAddr, _reg, _num);		  /* 发送命令 */
...
uint8_t MODH_ReadParam_03H(uint16_t _reg, uint16_t _num)
{
	...
	for (i = 0; i < NUM; i++)
	{
		MODH_Send03H (SlaveAddr, _reg, _num);		  /* 发送命令 */
...
uint8_t MODH_ReadParam_04H(uint16_t _reg, uint16_t _num)
{
	...
	for (i = 0; i < NUM; i++)
	{
		MODH_Send04H (SlaveAddr, _reg, _num);		  /* 发送命令 */
