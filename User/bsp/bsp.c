...
void bsp_Init(void)
{
	...
	*/
	...
	bsp_InitDWT();          /* 通过函数SystemCoreClockUpdate更新SystemCoreClock后才可以调用 */
	bsp_InitUart(); 	    /* 初始化串口 */
	bsp_InitKey();		    /* 初始化按键变量（必须在 bsp_InitTimer() 之前调用） */
	
	bsp_InitHardTimer();    /* 硬件定时器初始化，Modbus的字符间隔要使用 */
	...
	SD_Link_EXTIConfig();   /* 设置SD卡的检测引脚 */
	
	bsp_InitRTC();
	BEEP_InitHard();
	
	BSP_WM8978Init();      /* 初始WM8978 */
