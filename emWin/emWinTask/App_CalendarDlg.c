...
static void Clock_DrawDisp(void) 
{
	...
	/* 第一个指针计数，用于旋转秒针 */
	t0 = 360 - RTC_TimeStructure.RTC_Seconds * 6;

	/* 第二个指针计数，用于旋转分针 */
	t1 = 360 - RTC_TimeStructure.RTC_Minutes * 6;
	
	/* 第三个指针计数，用于旋转时针 */
	t2 = 360 - RTC_TimeStructure.RTC_Hours * 30;
...
static void Clock_Update(WM_MESSAGE * pMsg) 
{
	...
	/* 更新星期 */
	sprintf(buf, "星期: %s", ucWeekDay[RTC_DateStructure.RTC_WeekDay-1]);
